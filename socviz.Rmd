...
It no longer maps the individual points; instead it's calculated a smooth line with a shaded area equal to the standard error.  By default, the smoothed line is determined via *gam* method - genreal additive model.  
...
We can see wo bunched GDP is on the left hand side of the x-axis.  By applying a log scale, we can improve the clarity:  
```{r}
p +
...
The log xform is applied prior to the scalling function.  
...
2. then, we can give the *scale_* functions a *labels* argument.  
