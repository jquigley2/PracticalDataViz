...
Using functions to do calculations can get confusing, since the code is stuffed into the mapping function.  A better strategy is calculating the frequency table first, then plotting the result.  This also allows us to do some sanity checks on the table, rather than simply using the graphical output to find errors.
...
* `group_by()` Groups the data into a nested structure, such as religions by region, or authors by publications by year
...
1. Once created, objects are carried along the pipeline
2. The 'group_by()' function sets up how the data will be processed in the `summarize()` step; any function is applied to the innermost grouping level first.
  + grouping levels are named from left to right in the 'group_by()', from outermost to innermost.
    + In this case, `summarize()` is applied to religion (innermost) then region (outermost).
3. `mutate()` functions add or remove columns from the table, but do not change grouping.
4. Note, we can invent named variables with `summarize()` ("N") and `mutate()` ("freq", "pct").  We pick the names *AND* assign the variables.  These will be in the new summary table.
5. When we create the new `freq` variable in `mutate()`, it allows us to use the newly created variable in the very next function.  

It can be helpful to remove elements of the pipe to sanity-check our work.  For example, what do we see if we remover the `mutate()` function from the above pipe?  

We can also check whether or not each region's religions sum to 100% (or 1):
...
Now, since we are working with percentage proportions, we can use `geom_col()` instead of `geom_bar()`  
```{r}
p <- ggplot(rel_by_region, aes(x = bigregion, y = pct, fill = religion)) 
p + geom_col(position = "dodge2") +                                       #dodge2 inserts slightly more space in between bars
  labs(x = "Region", y = "Percent", fill = "Religion") +
  theme(legend.position = "top")

```
Dodged charts can better be shown as faceted plots.  
Mapping religion to the x-axis will result in a very crowded plot.  Even adjusting the labels to print at an angle makes this only slightly better. It's better to put the religions on the Y axis and percent scores on the y-axis.  In order to do this, we need to transform the *coordinate system* with `coord_flip()`:
```{r}
...
Here, we move to the `organdata` table.
...
```
Here's a scatterplot of donors versus year:
...
We can facet by country and look at a line graph of donors per year, per country:
```{r}
p <- ggplot(data = organdata,
            mapping = aes(x = year, y = donors))
p + geom_line(aes(group = country)) + 
...
If we instead want to focus on country-level variation - ignoring the time limit - a boxplot is useful.
```{r}
p <- ggplot(organdata,
...
This is a bit tough to read on the x-axis.  We're better off flipping this on its side using `coord_flip()`:
```{r}
p + geom_boxplot() +
  coord_flip()
```

This is better, but we should order by donors somehow.  We can re-order based on the mean number of donors per country.

`reorder()` takes 2 **required** arguments: first is the category to reorder, second is the variable to reorder by. A third, optional argument is the function used as the summary statistic.  Mean is the default here (could also use `median` or `sd`, for example). 
*Note: `mean` calculation will fail if there are NAs in the data set; remove these with `na.rm = TRUE`.*:
```{r}
p <- ggplot(data = organdata,
...
#Color the boxes by another variable
p <- ggplot(data = organdata,
            mapping = aes(x = reorder(country, donors, na.rm=TRUE),    
                          y = donors, fill = opt))
...
Since there are relatively few data points per country, we could also look at the individual data points.  
```{r}
p <- ggplot(data = organdata,
            mapping = aes(x = reorder(country, donors, na.rm=TRUE),    
                          y = donors, color = opt))
...
p + geom_point() +
  ...
  theme(legend.position = "top")
```
However, some of the data points overlap, so we can use `geom_jitter()` instead of `geom_point()`:
```{r}
p + geom_jitter() +
  labs(x = NULL) +
  coord_flip() +
  theme(legend.position = "top")
```
The jitter is a bit too much.  We can reduce both the horizontal (`width`) and vertical (`height`) jitter inside the `position_jitter()` function:
```{r}
...
We can use a similar approach when there is a single data point per categorical variable.  This is known as the Cleveland Dot Plot.  Let's try this for the average donation rate per country.  First, we'll need to develop that data with a pipe:
```{r}
...
Note the above pipe code requires us to type in code to calculate mean, sd, and remove NA's multiple times, plus repeatedly naming the new variables.  There's a better way to accomplish all of this.  We can apply the `mean()` and `sd()` functions to every **numerical** variable in `organdata`.  Here's the code:
```{r}
by_country <- organdata %>% group_by(consent_law, country) %>% 
  summarize_if(is.numeric, funs(mean,sd), na.rm = TRUE) %>%      #tests to see whether the vector is numeric.  If so, applies mean and sd functions, and removes all NAs.
  ungroup()
...
The summary data are named by appending the function name to the original variable, such as `donors_mean` and `donors_sd'.

*There are families of both `summarize_` and `mutate()` functions. (Reminder: `mutate()` functions add columns)*

Now, we can draw a dot plot of mean donors per country, and color the results by the consent law `consent_law` of each country:
```{r}
...
We could use a facet rather than coloring by consent law.  This would allow easy comparison, but we need to solve for a couple of problems:  
1. The default for faceting is across a row.  We want both facets to be in a single column. We can use `ncol = 1` to acheive this.
2. Because we have a categorical value on the y-axis, the default setting for facets will have every country on *both* plots.  So, only half the rows would have points.
  + To avoid this, allow the scales to be free via the `scales = free_y` argument.
```{r}
p <-  ggplot(data = by_country,
             mapping = aes(x = donors_mean,
                           y = reorder(country, donors_mean)))
...
We can also show information about variance on the plot by using `geom_pointrange()`, which gives us a point estimate and the range around it.  We map our `x` and `y` variables as usual, but must add `ymax` and `ymin` to set the range.  These are given by the mean (`donors_mean) +/- the standard deviation (`donors_sd`) in this instance):
```{r}
p <- ggplot(data = by_country, 
...
We can use `geom_text()` to plot all labels along with points, or just select points.   
```{r}
p <- ggplot(data = by_country,
...
Unfortunately, this plots right on top of the points, obscuring them.  It's better to use functions from the `ggrepel` library instead.  This library adds some useful new geoms to `ggplot`.  
...
There is also a `geom_abline()` that draws straight lines based on a supplied slope and intercept. This is useful for plotting, for example, 45 degree reference lines in scatterplots.  
  
### 5.4 Label Outliers  
We can also pick out specific points to repel, by choosing a different set than `geom_point()` uses.  For example:
...
We can also pick out specific points by creating a dummy variable in the data set for this purpose:
```{r}

organdata$ind <-                              #creates a dummy variable which will be true or false
  organdata$ccode %in% c("Ita", "Spa") &      #country code must be Ita or Spa AND
  organdata$year > 1998                       #year > 1998

p <- ggplot(data = organdata,
            mapping = aes(x = roads,
                          y = donors,
                          color = ind))     #map dummy to color
...
We use `annotate()` to highlight something not mapped to a variable.  We can use this for placing text on the plot, or highlighting an area.
